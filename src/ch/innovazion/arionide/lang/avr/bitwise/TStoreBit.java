...
public class TStoreBit extends Instruction {
	...
	public void evaluate(Environment env, Specification spec, ApplicationMemory programMemory) throws EvaluationException {		
		...
		int value = (sram.getRegister(dPtr) & mValue) != 0 ? 1 : 0;
		
		sram.set(AVRSRAM.SREG, (sram.get(AVRSRAM.SREG) & 0b10111111) | value);
	...
	public StructureModel createStructureModel() {
		return StructureModelFactory
			.draft("bst")
			.withColor(0.66f)
			.withComment("Stores the specified bit of a register into the T flag of SREG")
			.beginSignature("default")
			.withParameter(new Parameter("Source", AVREnums.REGISTER))
			.withParameter(new Parameter("Bit number", AVREnums.AND_BIT_MASK))
			.endSignature()
			.build();
	}
