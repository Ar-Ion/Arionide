...
public class UIMainView extends UIView implements UIButtonClickListener {
	
	public static int pageID = 0;
	
	public UIMainView(UIView parent, Frame rootComponent) {
		...
		if(UIMainView.pageID == 0) {
			...
			for(int i = 0; i < 4; i++) {
				try {
					String project = Projects.getProjectsList().get((UIMainView.pageID - 1) * 4 + i);
		...
		if(Projects.getProjectsList().size() > UIMainView.pageID * 4) {
	...
	public static void updatePageID() {
		UIMainView.pageID = (int) Math.ceil(Projects.getProjectsList().size() / 4.0d);
	}
}